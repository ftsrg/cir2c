...

{
  ...
  for (i = 0; i < w; i++) {
    for (j = 0; j < h; j++)
      print_element(imgOut[i][j], j*16, printmat);
...
static inline int
check_FP(int n,
         DATA_TYPE POLYBENCH_2D(A,W,H,w,h),
         DATA_TYPE POLYBENCH_2D(B,W,H,w,h)) {
  int i, j;
...
#if !FMA_DISABLED
  POLYBENCH_2D_ARRAY_DECL(imgOut_StrictFP, DATA_TYPE, W, H, w, h);
#endif
...
#else
    init_array (w, h, &alpha, POLYBENCH_ARRAY(imgIn), POLYBENCH_ARRAY(imgOut_StrictFP));
    kernel_deriche_StrictFP (w, h, alpha, POLYBENCH_ARRAY(imgIn), POLYBENCH_ARRAY(imgOut_StrictFP), POLYBENCH_ARRAY(y1), POLYBENCH_ARRAY(y2));
    if (!check_FP(w, POLYBENCH_ARRAY(imgOut), POLYBENCH_ARRAY(imgOut_StrictFP)))
      return 1;
    /* Prevent dead-code elimination. All live-out data must be printed
       by the function call in argument. */
    polybench_prevent_dce(print_array(w, h, POLYBENCH_ARRAY(imgOut_StrictFP)));
#endif
...
#if !FMA_DISABLED
  POLYBENCH_FREE_ARRAY(imgOut_StrictFP);
#endif
