...
public:
  ...
  /// Emit the top-of-file header into `out`: a "Generated by cir2c" banner plus
  /// a "Verifier should support: ..." manifest of the non-trivial C constructs
  /// the output actually uses. The manifest is computed from the emitted output:
  /// op-based constructs (indexing, pointer casts, struct access, builtins, ...)
  /// are detected by scanning `module`, while declaration-driven features
  /// (memcpy, packed, __VERIFIER_nondet_memory havoc, ...) are read from the
  /// flags set during body emission. Call only for the top-level module, after
  /// the body has been emitted so those flags are populated.
  void emitFileHeader(std::ostream &out, mlir::ModuleOp module);
